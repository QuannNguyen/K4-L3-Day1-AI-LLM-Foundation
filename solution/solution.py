...
def call_openai(
    ...
    return text, max(0.0, float(end - start))
...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost_estimate = (
        max(len(gpt4o_response.split()) / 0.75, 1) / 1000
        * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    )
...
if __name__ == "__main__":
    print("=== So sánh model ===")
    # result = chat_with_system_prompt("Bạn là giáo viên tiểu học, giải thích thật đơn giản cho trẻ 8 tuổi.","Giải thích blockchain là gì?")
    # result = chat_with_system_prompt("Bạn là chuyên gia tài chính, trả lời chuyên sâu bằng thuật ngữ kỹ thuật.","Giải thích blockchain là gì?")
    ...
    for key, value in result.items():
        print(f"{key}: {value}")
    # count = count_tokens("Người giúp đỡ trong cộng đồng mà tôi yêu quý nhất là cô Lan – y tá ở trạm y tế xã. Cô luôn tận tâm chăm sóc bệnh nhân, dù trời nắng hay mưa. Mỗi khi có người bị ốm, cô đều đến tận nhà thăm hỏi, mang theo thuốc men và lời động viên ấm áp. Cô không chỉ chữa bệnh mà còn hướng dẫn mọi người cách giữ gìn sức khỏe. Nụ cười hiền hậu và giọng nói nhẹ nhàng của cô khiến ai cũng cảm thấy yên tâm. Nhờ có cô, nhiều người trong làng khỏe mạnh hơn và sống vui vẻ, đoàn kết hơn.")
